...
[BCs]
  [./Periodic]
    ...
    [./xy]
      auto_direction = 'x'
      variable = 'eta0 eta1 eta2 eta3 eta4 c w c0 c1 c3'
    [../]
  [../]
...
[Materials]
  ...
  [./nuc]
    ...
    f_name = nuc
    material_property_names = 'time dt T kb lambda dim L h0(eta0,eta1,eta2,eta3,eta4) h1(eta0,eta1,eta2,eta3,eta4) h2(eta0,eta1,eta2,eta3,eta4) h3(eta0,eta1,eta2,eta3,eta4)'
    function = 'if(time<1&h0*(h1+h2+h3)>0.09,sqrt(2*kb*T*L/(lambda^dim*dt)),0)' #expression from Shen (2007)
    outputs = exodus
  ...
  [./Mgb]
    ...
    material_property_names = 'D_gb delta delta_real h0 h1 h2 h3 h4 A_cu A_imc A_sn length_scale energy_scale time_scale'
    #material_property_names = 'D_gb delta delta_real h0(eta0,eta1,eta2,eta3,eta4) h1(eta0,eta1,eta2,eta3,eta4) h2(eta0,eta1,eta2,eta3,eta4) h3(eta0,eta1,eta2,eta3,eta4) h4(eta0,eta1,eta2,eta3,eta4) A_cu A_imc A_sn length_scale energy_scale time_scale'
    f_name = Mgb
    #function = '(length_scale^5/(energy_scale*time_scale))*3.*D_gb*delta_real/((h0*A_cu+h1*A_imc+(h2+h3)*A_sn)*delta)'
    function = 'if(h1*(h2+h3)+h2*h3>0.09,(length_scale^5/(energy_scale*time_scale))*3.*D_gb*delta_real/((h0*A_cu+h4*A_imc+(h3+h2+h1)*A_sn)*delta),0)'
    #function = '4e-5'
    outputs = exodus
  ...
  [./CHMobility]
      ...
      f_name = M
      #args = 'eta0 eta1 eta2 eta3 eta4'
      #material_property_names = 'h0(eta0,eta1,eta2,eta3,eta4) h1(eta0,eta1,eta2,eta3,eta4) h2(eta0,eta1,eta2,eta3,eta4) h3(eta0,eta1,eta2,eta3,eta4) h4(eta0,eta1,eta2,eta3,eta4) D_cu D_eta D_sn A_cu A_eta A_sn Mgb(eta0,eta1,eta2,eta3,eta4) length_scale energy_scale time_scale'
      #material_property_names = 'h0 h1 h2 h3 h4 D_cu D_eta D_sn A_cu A_eta A_sn Mgb length_scale energy_scale time_scale'
      ...
      #function = '(length_scale^5/(energy_scale*time_scale))*(h0*D_cu/A_cu+h4*D_eta/A_eta+(h3+h2+h1)*D_sn/A_sn)+Mgb' #nm^5/eVs
      function = '650'
...
[AuxKernels]
  ...
  [./f_int]
    ...
    function ='mu:=(6*sigma/delta)*(energy_scale/length_scale^3); mu*(0.25*eta0^4-0.5*eta0^2+0.25*eta1^4-0.5*eta1^2+0.25*eta2^4-0.5*eta2^2+0.25*eta3^4-0.5*eta3^2+0.25*eta4^4+0.5*eta4^2+gamma*(eta0^2*(eta1^2+eta2^2+eta3^2+eta4^2)+eta1^2*(eta1^2+eta2^2+eta3^2+eta4^2)+eta2^2*(eta3^2+eta4^2)+eta3^2*eta4^2)+0.25)'
    execute_on = 'initial timestep_end'
    use_displaced_mesh = false
  [../]
[]
