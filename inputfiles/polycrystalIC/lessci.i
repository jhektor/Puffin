...
[UserObjects]
  [./voronoi]
    ...
    variable = 'eta0 eta1 eta2 eta3 eta4'
    rand_seed = 77665543
...
[ICs]
  ...
  [./c] #Concentration of Sn
    type = VarDepIC
    variable = c
    cis = 'c0 c1 c1 c1 c1 c2'
    etas = 'eta0 eta1 eta2 eta3 eta4 eta5'
  [../]
  #[./w]
  #  type = FunctionIC
  #  variable = w
...
[BCs]
  [./Periodic]
    ...
    [./xy]
      auto_direction = 'x'
      variable = 'eta0 eta1 eta2 eta3 eta4 eta5 c w c0 c1 c2'
    [../]
  [../]
...
[Materials]
  ...
  [./h0]
      ...
      h_name = h0
      all_etas = 'eta0 eta1 eta2 eta3 eta4 eta5'
      phase_etas = eta0
      use_displaced_mesh = false
      outputs = exodus
  ...
  [./nuc]
    ...
    f_name = nuc
    material_property_names = 'time dt T kb lambda dim L h0(eta0,eta1,eta2,eta3,eta4,eta5) h1(eta0,eta1,eta2,eta3,eta4,eta5) h2(eta0,eta1,eta2,eta3,eta4,eta5) h3(eta0,eta1,eta2,eta3,eta4,eta5) h4(eta0,eta1,eta2,eta3,eta4,eta5)'
    function = 'if(time<1&h0*(h1+h2+h3+h4)>0.09,sqrt(2*kb*T*L/(lambda^dim*dt)),0)' #expression from Shen (2007)
    outputs = exodus
  ...
  [./Mgb]
    type=DerivativeParsedMaterial
    args = 'eta0 eta1 eta2 eta3 eta4 eta5'
    #material_property_names = 'D_gb delta delta_real h0(eta0,eta1,eta2,eta3,eta4,eta5) h1(eta0,eta1,eta2,eta3,eta4,eta5) h2(eta0,eta1,eta2,eta3,eta4,eta5) h3(eta0,eta1,eta2,eta3,eta4,eta5) h4(eta0,eta1,eta2,eta3,eta4,eta5) h5(eta0,eta1,eta2,eta3,eta4,eta5) A_cu A_imc A_sn length_scale energy_scale time_scale'
    material_property_names = 'D_gb delta delta_real h0 h1 h2 h3 h4 h5 A_cu A_imc A_sn length_scale energy_scale time_scale'
    #material_property_names = 'D_gb delta delta_real h0(eta0,eta1,eta2,eta3,eta4,eta5) h1(eta0,eta1,eta2,eta3,eta4,eta5) h2(eta0,eta1,eta2,eta3,eta4,eta5) h3(eta0,eta1,eta2,eta3,eta4,eta5) h4(eta0,eta1,eta2,eta3,eta4,eta5) h5(eta0,eta1,eta2,eta3,eta4,eta5) A_cu A_imc A_sn length_scale energy_scale time_scale'
    f_name = Mgb
    #function = '(length_scale^5/(energy_scale*time_scale))*3.*D_gb*delta_real/((h0*A_cu+h1*A_imc+(h2+h3)*A_sn)*delta)'
    function = 'if(h1*(h2+h3+h4)+h2*(h3+h4)+h3*h4>0.09,(length_scale^5/(energy_scale*time_scale))*3.*D_gb*delta_real/((h0*A_cu+h5*A_imc+(h2+h1+h3+h4)*A_sn)*delta),0)'
    #function = '4e-5'
    outputs = exodus
  ...
  [./CHMobility]
      ...
      f_name = M
      args = 'eta0 eta1 eta2 eta3 eta4 eta5'
      material_property_names = 'h0 h1 h2 h3 h4 h5 D_cu D_eta D_sn A_cu A_eta A_sn Mgb length_scale energy_scale time_scale'
      #material_property_names = 'h0(eta0,eta1,eta2,eta3,eta4,eta5) h1(eta0,eta1,eta2,eta3,eta4,eta5) h2(eta0,eta1,eta2,eta3,eta4,eta5) h3(eta0,eta1,eta2,eta3,eta4,eta5) h4(eta0,eta1,eta2,eta3,eta4,eta5) h5(eta0,eta1,eta2,eta3,eta4,eta5) D_cu D_eta D_sn A_cu A_eta A_sn Mgb(eta0,eta1,eta2,eta3,eta4,eta5) length_scale energy_scale time_scale'
      ...
      function = '(length_scale^5/(energy_scale*time_scale))*(h0*D_cu/A_cu+h5*D_eta/A_eta+(h4+h3+h2+h1)*D_sn/A_sn)+Mgb' #nm^5/eVs
      #function = '200' #nm^5/eVs
...
[Kernels]
  ...
  [./CHBulk] # Gives the residual for the concentration, dF/dc-mu
      ...
      ca       = c2
      cb       = c1
      fa_name  = F5 #only fa is used
      fb_name  = F1
      w        = w
      h_name   = h5
      args_a = 'eta0 eta1 eta2 eta3 eta4 eta5'
      use_displaced_mesh = false
  [../]
  ...
  [./chempot_cu_eta]
    ...
    use_displaced_mesh = false
    args_a = ' '
    args_b = 'eta0 eta1 eta2 eta3 eta4 eta5'
  [../]
  [./chempot_eta_sn]
    type = KKSPhaseChemicalPotential
    variable = c1
  ...
  [./phaseconcentration] # enforce c = sum h_i*c_i
    ...
    variable = c2
    cj = 'c0 c1 c1 c1 c1 c2'
    hj_names = 'h0 h1 h2 h3 h4 h5'
    etas = 'eta0 eta1 eta2 eta3 eta4 eta5'
    c = c
    use_displaced_mesh = false
  [../]
...
[AuxKernels]
  [./f_density]
    ...
    variable = f_density
    hj_names = 'h0 h1 h2 h3 h4 h5'
    Fj_names = 'F0 F1 F2 F3 F4 F5'
    gj_names = 'g0 g1 g2 g3 g4 g5'
    additional_free_energy = f_int
    interfacial_vars = 'eta0 eta1 eta2 eta3 eta4 eta5'
  ...
  [./f_int]
    ...
    function ='mu:=(6*sigma/delta)*(energy_scale/length_scale^3); mu*(0.25*eta0^4-0.5*eta0^2+0.25*eta1^4-0.5*eta1^2+0.25*eta2^4-0.5*eta2^2+0.25*eta3^4-0.5*eta3^2+0.25*eta4^4-0.5*eta4^2+0.25*eta5^4-0.5*eta5^2+gamma*(eta0^2*(eta1^2+eta2^2+eta3^2+eta4^2+eta5^2)+eta1^2*(eta1^2+eta2^2+eta3^2+eta4^2+eta5^2)+eta2^2*(eta3^2+eta4^2+eta5^2)+eta3^2*(eta4^2+eta5^2)+eta4^2*eta5^2)+0.25)'
    execute_on = 'initial timestep_end'
    use_displaced_mesh = false
  [../]
[]
...
[Preconditioning]
  [./smp]
    ...
    #petsc_options_iname = '-pc_asm_overlap -pc_type -sub_pc_type  -pc_factor_shift_type  -sub_pc_factor_shift_type -sub_ksp_type'
    #petsc_options_value = '2                  asm       ilu             nonzero             nonzero                    preonly'
