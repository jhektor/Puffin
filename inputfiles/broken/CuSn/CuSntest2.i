...
[ICs]
    ...
    [./c] #Concentration of Sn
        ...
        type = FunctionIC
        function = '(0.01*if(y<=10,1,0)+0.417*if(y>10&y<=18,1,0)+0.99*if(y>18,1,0))/16.29e12' #[mol/um^3] #TODO: Make nicer, should be possible to use values of the other variables.
...
[Materials]
    ...
    [./fch_cu] #Chemical energy Cu phase
        ...
        function = '(0.5*A_cu*(c_cu-chat_cu)^2+B_cu*(c_cu-chat_cu)+Cc_cu)/Vm_cu' #[J/um^3]
    [../]
    ...
    [./CHMobility]
        ...
        f_name = M
        args = 'eta_cu eta_imc eta_sn'
        material_property_names = 'h_cu h_imc h_sn D_cu D_imc D_sn Vm_cu Vm_imc Vm_sn d2f_cu:=D[fch_cu(c_cu),c_cu,c_cu] d2f_imc:=D[fch_imc(c_imc),c_imc,c_imc] d2f_sn:=D[fch_sn(c_sn),c_sn,c_sn]'
        function = 'h_cu*D_cu/d2f_cu/Vm_cu+h_imc*D_imc/d2f_imc/Vm_imc+h_sn*D_sn/d2f_sn/Vm_sn'
    [../]
    [./ACMobility] #not quite correct
        type = ParsedMaterial
        f_name = L
        material_property_names = 'm alpha D_cu D_imc D_sn Vm_cu Vm_imc Vm_sn A_cu A_imc A_sn'
        function = 'm*(Vm_cu*D_cu/A_cu+Vm_imc*D_imc/A_imc+Vm_sn*D_sn/A_sn)/alpha'
        outputs = exodus
    [../]
[]
