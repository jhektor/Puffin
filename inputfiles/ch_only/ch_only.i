...
[ICs]
    ...
    [./c] #Concentration of Sn
        ...
        function = '0.2*if(sqrt((x-20)^2+(y-20)^2)<=8,1,0)+0.5*if(sqrt((x-20)^2+(y-20)^2)>8&sqrt((x-20)^2+(y-20)^2)<=16,1,0)+0.85*if(sqrt((x-20)^2+(y-20)^2)>16,1,0)' #TODO: Make nicer, should be possible to use values of the other variables.
    [../]
[]
...
[Kernels]
    ...
    [./dcdt]
        ...
        v = c
    [../]
    [./ckernel] # Gives residual for chemical potential dcdt+M\grad(mu)
        type = SplitCHWRes
        mob_name = M
        variable = w
    [../]
