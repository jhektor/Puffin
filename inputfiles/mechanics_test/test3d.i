...
[BCs]
  ...
  [./Periodic]
    [./all]
      auto_direction = 'x z'
      variable = 'c w c0 c1 c2 eta0 eta1 eta2' # disp_x disp_y disp_z'
    [../]
  [../]
...
[Variables]
    ...
    # OBS TRY TO FIND REASON FOR "poor" CONVERGENCE RATHER THAN SCALING RESIDUAL
    #[./disp_x]
    #  order = FIRST
    #  family = LAGRANGE
    #  #scaling = 1e-3
...
[Materials]
  ...
  [./nuc_eta]
    ...
    material_property_names = 'dt T kb lambda dim L h0 h2'
    #function = '80*h0*h2'
    function = 'sqrt(2*kb*T*L/(lambda^dim*dt))*h0*h2' #expression from Shen (2007) (without h0*h2)
    #function = 'if(h0*h2>=0.09,200*sqrt(2*kb*T*L/(lambda^dim*dt)),0)' #expression from Shen (2007) (without h0*h2) nucleation between eta0,eta2 [0.1,0.9]
    #function = 'if(h0*h2>0,sqrt(2*kb*T*L/(lambda^dim*dt)),0)' #expression from Shen (2007) (without h0*h2) nucleation between eta0,eta2 [0.,1]
    #function = '100'
    outputs = exodus_out
  ...
  #  type = ParsedMaterial
  #  args = 'eta0 eta1 eta2'
  #  material_property_names = 'time h1(eta0,eta1,eta2)'
  #  f_name = pre
...
[Outputs]
  file_base = 3d-40x60x1-estar0-test
  [./exodus_out]
    type = Exodus
    interval =1
  [../]
