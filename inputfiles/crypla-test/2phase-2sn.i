...
[Functions]
  ...
  [./teta1]
    type = ParsedFunction
    value = 'r:=sqrt(x^2+y^2);if(0.5*(1+tanh(x/60))-1+0.5*(1+tanh((r-200*t)/60))>0,0.5*(1+tanh(x/60))-1+0.5*(1+tanh((r-200*t)/60)),0)'
  [../]
  [./teta2]
    type = ParsedFunction
    #value = 'r:=sqrt(x^2+y^2);if(r<=0.1*t,0,1)'
    value = 'r:=sqrt(x^2+y^2);1-0.5*(1+tanh((r-200*t)/60))'
  [../]
[]
