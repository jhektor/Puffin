...
[Variables]
    ...
    [./c_sn]
        order = FIRST
        family = LAGRANGE
        initial_condition = 0.999 #0.95#0.999
    [../]
...
[ICs]
    ...
    [./c] #Concentration of Sn
        variable = c
        type = FunctionIC
        function = 'if(x<=48500,0.05,0)+if(x>48500&x<=51500,0.455,0)+if(x>51500,0.999,0)'
    [../]
[]
...
[Materials]
    ...
    [./ACMobility]
        ...
        #function ='numer:=L_cu_imc*eta_cu^2*eta_imc^2+L_imc_sn*eta_imc^2*eta_sn^2+L_cu_sn*eta_cu^2*eta_sn^2;denom:=eta_cu^2*eta_imc^2+eta_imc^2*eta_sn^2+eta_cu^2*eta_sn^2;if(denom!=0,100*numer/denom,0)'
...
[Outputs]
  file_base = keep/moelans2011fig3_Lvar_Mvar_sharp_barrier3
  [./exodus_out]
    type = Exodus
    interval = 1
  [../]
