...
[ICs]
    [./eta1] #Cu
        variable = eta_cu
        type = FunctionIC
        function = 'if(x<=23000,1,0)'
    [../]
    [./eta2] #Cu6Sn5
        variable = eta_imc
        type = FunctionIC
        function = 'if(x>23000&x<=27000,1,0)'
    [../]
    [./eta3] #Sn
        variable = eta_sn
        type = FunctionIC
        function = 'if(x>27000,1,0)'
    [../]

    [./c] #Concentration of Sn
        variable = c
        type = FunctionIC
        function = 'if(x<=23000,0.95,0)+if(x>23000&x<=27000,0.455,0)+if(x>27000,0.95,0)'
    [../]
[]
...
[Materials]
    ...
    [./energy_constants]
        ...
        prop_values = '1e9 1e9 1e9 0.999 0.4760 0.4760 0.999 0.978 0.978' #J/m^3  -
        #prop_values = '1e9 1e9 1e9 0.0504 0.4524 0.4760 0.999 0.1938 0.9898' #J/m^3  -
    [../]
    [./diffusion_constants]
      type = GenericConstantMaterial
      prop_names = 'D_cu D_imc D_sn'
      prop_values = '1e-14 1e-16 1e-14' # m^2/s
    ...
    [./L_cu_sn]
      ...
      f_name = L_cu_sn
      function = '0' #(length_scale^5/(energy_scale*time_scale))*2*mu*(D_sn/A_sn+D_cu/A_cu)/(3*kappa*(c_sn_cu-c_cu_sn)^2)'
    [../]
    #Free energy
    [./fch_cu] #Chemical energy Cu phase
        ...
        function = '(energy_scale/length_scale^3)*(0.5*A_cu*(c_cu-0.978)^2+1e7)' #eV/nm^3
        derivative_order = 2
    [../]
    ...
    [./ACMobility]
        ...
        function ='pf:=1e5;eps:=0.01;if(eta_cu>0.999|eta_imc>0.999|eta_sn>0.999,0,(L_cu_imc*(pf*eta_cu^2+eps)*(pf*eta_imc^2+eps)+L_imc_sn*(pf*eta_imc^2+eps)*(pf*eta_sn^2+eps)+L_cu_sn*(pf*eta_cu^2+eps)*(pf*eta_sn^2+eps))/((pf*eta_cu^2+eps)*(pf*eta_imc^2+eps)+(pf*eta_imc^2+eps)*(pf*eta_sn^2+eps)+(pf*eta_cu^2+eps)*(pf*eta_sn^2+eps)))'
