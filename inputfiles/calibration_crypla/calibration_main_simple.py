...
def calibfcn(x, pltstring = '--'):
    ...
    for loadcase in [0]:
        #Read experimental data from mat files change back angles to 90 90 0 in 0!!!
        if loadcase is 0:
        ...
        slip_rate_props_vals = [1, 4, x[0], x[1], 5, 8, x[6], x[7], 9, 32, x[8], x[9]] #start_ss end_ss gamma0 1/m m = 20??
        state_var_props_vals = [x[2]]*6# initial slip resistance values of each ss
        state_var_rate_h0_vals =  [x[3]]*6 #  h0 of each ss
        state_var_rate_tauSat_vals = [x[4]]*6 # tau saturation of each ss
        state_var_rate_hardeningExponent_vals = [x[5]]*6 # the hardening exponent c
...
#mpar = np.array([0.1]*10) # --- TODO insert result from an optimization
#mpar[0:2] = [1e-3, 5e-2]
#mpar[6:8] = [1e-3, 5e-2]
#mpar[8:10] = [1e-3, 5e-2]
#mpar[2] = 0.18
...
mpar = ([9.83740267e-02, 9.76149683e-02, 2.06033163e-02, 1.69950270e-01, 
         1.05343594e-01, 2.94480932e+00, 1.00000000e-03, 5.00000000e-02,
         9.88659353e-02, 9.75246064e-02])
