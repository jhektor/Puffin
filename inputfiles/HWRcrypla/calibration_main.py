...
def calibfcn(x, pltstring = '--'):
    ...
    for loadcase in [0,1]:
        ...
        scaling = [0.001, 6, 7e-3, 8]
        slip_rate_props_gamma0 = x[0]*scaling[0] #0.001
        slip_rate_props_m = x[1]*scaling[1] #6
        state_var_props = x[2]*scaling[2] #7e-3 # initial slip resistance values of each ss
        state_var_rate_B =  x[3]*scaling[3] #8.0
        ...
        runcmd = 'mpirun -n 1 ../../puffin-opt -i ' + inputfile + slip_rate_props_gamma0_name + str(slip_rate_props_gamma0)+ ' ' + slip_rate_props_m_name + str(slip_rate_props_m)+ ' ' + state_var_props_name + str(state_var_props)+ ' ' + state_var_rate_B_name + str(state_var_rate_B)+ ' ' + eul2 + ' > mooselog.txt'
