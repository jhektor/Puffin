...
\end{equation}
where $\vbf{T}$ is the second Piola-Kirchoff stress tensor, $\vbf{E}^e$ is the elastic Green strain tensor, $Q$ is a hardening parameter, and $s^\alpha$ is the slip resistance on the $\alpha$ slip system and the sum is taken over all $n$ slip systems. \todo{It should be the same elastic energy for all phases} \todo{$Q$ should be related to something in the equations for $\gamma$ or $s$}
