...
Since there is no dependence on $\gradient c$ in the free energy the divergence term of \eqref{eq:CH1} vanishes.\footnote{This is only true for KKS models, in other multiphase models a term like $\kappa\left(\gradient c\right)^2$ is included.
This term will make the Cahn-Hilliard a fourth order PDE.}

Using \eqref{eq:F}, \eqref{eq:fch} and \eqref{eq:fint} the derivative become
\begin{equation}
  \partdiff{F}{c}=\partdiff{f_{ch}}{c}=\sum_i h_i\partdiff{f_i}{x_i}\partdiff{x_i}{c}+\cancelto{0}{\partdiff{h_i}{c}f_i}.
  \label{eq:dFdc1}
\end{equation}
By taking the derivative of the mass conservation \eqref{eq:mass} we get
\begin{equation}
  \partdiff{c}{c}=\sum_i h_i\partdiff{x_i}{c}=1.
\end{equation}
Inserting into \eqref{eq:dFdc1} and taking into account \eqref{eq:KKS} we end up with
\begin{equation}
  \partdiff{F}{c}=\mu,
\end{equation}
where we can arbitrarily choose $\mu=\mu_i$ based on the KKS condition.
With this \eqref{eq:CH1} can be written as
\begin{equation}
...
\label{eq:AC1}
\end{equation}
The derivatives of the interface energy are easy to calculate from \eqref{eq:fint}
\begin{equation}
...
\end{equation}
In the case where the molar volume is assumed constant the first term of \eqref{eq:dfjdetai} vanishes.
For the second term we can use that $\partdiff{c}{\eta_i}=0$, equation \eqref{eq:mass} then gives
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation} 
By making use of the KKS condition \eqref{eq:dfchdeta} can then be written
\begin{equation}
...
Equation \eqref{eq:AC1} now becomes
\begin{multline}
...
The weak form of the residual equation for $\eta_i$ is constructed from \eqref{eq:AC1} by multiplication with a test function $\psi$ and integrating over the domain:
\begin{equation}
\begin{aligned}
...
The Cahn-Hilliard equation \eqref{eq:CH} can be solved in two ways.
