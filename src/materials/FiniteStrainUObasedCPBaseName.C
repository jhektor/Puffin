...
template <>
InputParameters
validParams<FiniteStrainUObasedCPBaseName>()
{
...
FiniteStrainUObasedCPBaseName::FiniteStrainUObasedCPBaseName(const InputParameters & parameters)
  : ComputeStressBase(parameters),
    ...
    _fp(declareProperty<RankTwoTensor>(_base_name + "fp")), // Plastic deformation gradient
    _fp_old(getMaterialPropertyOld<RankTwoTensor>(_base_name +
        "fp")), // Plastic deformation gradient of previous increment
    _pk2(declareProperty<RankTwoTensor>(_base_name + "pk2")), // 2nd Piola Kirchoff Stress
    ...
    _crysrot(getMaterialProperty<RankTwoTensor>(_base_name + "crysrot"))
{
  ...
  {
    _uo_slip_rates[i] = &getUserObjectByName<CrystalPlasticitySlipRateBaseName>(
...
FiniteStrainUObasedCPBaseName::computeQpStress()
{
  ...
  do
  {
    ...
    if (substep_iter > _max_substep_iter && _err_tol)
      throw MooseException("FiniteStrainUObasedCPBaseName: Constitutive failure.");
  } while (_err_tol);
...
void
FiniteStrainUObasedCPBaseName::solveQp()
{
  preSolveStatevar();
  solveStatevar();
  if (_err_tol)
...
void
FiniteStrainUObasedCPBaseName::preSolveStatevar()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
...
void
FiniteStrainUObasedCPBaseName::solveStatevar()
{
  unsigned int iterg;
  bool iter_flag = true;
  ...
  {
#ifdef DEBUG
    mooseWarning("FiniteStrainUObasedCPBaseName: Hardness Integration error\n");
#endif
    _err_tol = true;
  }
}
...
bool
FiniteStrainUObasedCPBaseName::isStateVariablesConverged()
{
  Real diff;
...
void
FiniteStrainUObasedCPBaseName::postSolveStatevar()
{
  // TODO: remove this nasty const_cast if you can figure out how
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
    const_cast<MaterialProperty<std::vector<Real>> &>(*_mat_prop_state_vars_old[i])[_qp] =
...
void
FiniteStrainUObasedCPBaseName::preSolveStress()
{
}

void
FiniteStrainUObasedCPBaseName::solveStress()
{
  unsigned int iter = 0;
  RankTwoTensor dpk2;
  ...
  {
#ifdef DEBUG
    mooseWarning("FiniteStrainUObasedCPBaseName: Slip increment exceeds tolerance - Element number ",
                 _current_elem->id(),
                 " Gauss point = ",
                 _qp);
#endif
  ...
  while (rnorm > _rtol * rnorm0 && rnorm0 > _abs_tol && iter < _maxiter)
  {
    ...
    {
#ifdef DEBUG
      mooseWarning("FiniteStrainUObasedCPBaseName: Failed with line search");
#endif
      _err_tol = true;
      return;
    }
  ...
  {
#ifdef DEBUG
    mooseWarning("FiniteStrainUObasedCPBaseName: Stress Integration error rmax = ", rnorm);
#endif
    _err_tol = true;
  }
}
...
void
FiniteStrainUObasedCPBaseName::postSolveStress()
{
  _fp[_qp] = _fp_inv.inverse();
}

void
FiniteStrainUObasedCPBaseName::updateSlipSystemResistanceAndStateVariable()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
...
void
FiniteStrainUObasedCPBaseName::calcResidJacob()
{
  calcResidual();
  if (_err_tol)
    return;
...
void
FiniteStrainUObasedCPBaseName::getSlipRates()
{
  for (unsigned int i = 0; i < _num_uo_slip_rates; ++i)
  {
...
void
FiniteStrainUObasedCPBaseName::calcTangentModuli()
{
...
void
FiniteStrainUObasedCPBaseName::elastoPlasticTangentModuli()
{
...
void
FiniteStrainUObasedCPBaseName::elasticTangentModuli()
{
...
bool
FiniteStrainUObasedCPBaseName::lineSearchUpdate(const Real rnorm_prev, const RankTwoTensor dpk2)
{
