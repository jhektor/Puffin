...
template <>
InputParameters
validParams<CrystalPlasticitySlipRateBaseName>()
{
...
CrystalPlasticitySlipRateBaseName::CrystalPlasticitySlipRateBaseName(const InputParameters & parameters)
  : CrystalPlasticityUOBase(parameters),
    ...
    _crysrot(getMaterialPropertyByName<RankTwoTensor>(_base_name + "crysrot"))
{
  getSlipSystems();
}

void
CrystalPlasticitySlipRateBaseName::readFileFlowRateParams()
{
}

void
CrystalPlasticitySlipRateBaseName::getFlowRateParams()
{
}

void
CrystalPlasticitySlipRateBaseName::getSlipSystems()
{
  ...
  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    ...
    for (unsigned int j = 0; j < LIBMESH_DIM; ++j)
      if (!(fileslipsys >> vec[j]))
        mooseError(
            "CrystalPlasticitySlipRateBaseName Error: Premature end of file reading slip system file \n");
    ...
    if (std::abs(mag) > 1e-8)
      mooseError("CrystalPlasticitySlipRateBaseName Error: Slip direction and normal not orthonormal, "
                 "System number = ",
                 i,
                 "\n");
  }
