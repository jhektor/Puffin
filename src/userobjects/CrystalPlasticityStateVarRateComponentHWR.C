...
validParams<CrystalPlasticityStateVarRateComponentHWR>()
{
  ...
  params.addParam<std::string>("uo_slip_resistance_name",
                               ...
                               "file.");
  params.addParam<Real>("B",1.0, "Material property for saturation of g");
    ...
    unsigned int qp, std::vector<Real> & val) const
{
  ...
  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    ...
    val[i] = (1.0 - _B*_mat_prop_state_var[qp][i])*(std::abs(tau)/_mat_prop_slip_res[qp][i])*std::abs(_mat_prop_slip_rate[qp][i]);
  }
  return true;
}
