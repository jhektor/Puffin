...
template <>
InputParameters
validParams<CrystalPlasticitySlipRateGSSBaseName>()
{
  InputParameters params = validParams<CrystalPlasticitySlipRateBaseName>();
...
CrystalPlasticitySlipRateGSSBaseName::CrystalPlasticitySlipRateGSSBaseName(const InputParameters & parameters)
...
void
CrystalPlasticitySlipRateGSSBaseName::readFileFlowRateParams()
{
  ...
  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    for (unsigned int j = 0; j < _num_slip_sys_flowrate_props; ++j)
      if (!(file >> vec[j]))
        mooseError(
            "Error CrystalPlasticitySlipRateGSSBaseName: Premature end of slip_sys_flow_rate_param file");
...
void
CrystalPlasticitySlipRateGSSBaseName::getFlowRateParams()
{
  if (_flowprops.size() <= 0)
    mooseError("CrystalPlasticitySlipRateGSSBaseName: Error in reading flow rate  properties: Specify "
  ...
  for (unsigned int i = 0; i < _flowprops.size() / num_data_grp; ++i)
  {
    ...
    if (vs <= 0 || ve <= 0)
      mooseError("CrystalPlasticitySlipRateGSSBaseName: Indices in flow rate parameter read must be "
    ...
    if (vs != std::floor(vs) || ve != std::floor(ve))
      mooseError("CrystalPlasticitySlipRateGSSBaseName: Error in reading flow props: Values specifying "
    ...
    if (is > ie)
      mooseError("CrystalPlasticitySlipRateGSSBaseName: Start index is = ",
                 is,
  ...
  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    ...
    {
      mooseWarning(
          "CrystalPlasticitySlipRateGSSBaseName: Non-positive flow rate parameters ", _a0(i), ",", _xm(i));
      break;
    }
  }
}
...
void
CrystalPlasticitySlipRateGSSBaseName::calcFlowDirection(unsigned int qp,
...
bool
CrystalPlasticitySlipRateGSSBaseName::calcSlipRate(unsigned int qp, Real dt, std::vector<Real> & val) const
{
...
bool
CrystalPlasticitySlipRateGSSBaseName::calcSlipRateDerivative(unsigned int qp,
                                                     Real /*dt*/,
